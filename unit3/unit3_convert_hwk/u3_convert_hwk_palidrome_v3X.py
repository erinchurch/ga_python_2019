...
def user_input(): #collect input from user, string
    x = input("Please provide the input you wish to test as a Palidrome.\t")
    return x


def remove_punct(x): #convert to have no punctuation, store in temporary string
    s_user = user_input()
...
def palidrome_compare(): #test if lower case no punctuation and lower case reversed are the same
    s_user = user_input()
    s_no_punct = remove_punct(s_user)
